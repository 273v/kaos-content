...
These standards apply to Python code in `kaos-content`.

`kaos-content` publishes the `kaos_content` import package. This package does not currently publish a CLI entry point.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-content` publishes import package `kaos_content`.
- Declare the public API in `kaos_content.__all__` when the package has
...
- Names exported from `kaos_content.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `dedup-perceptual`, `duckdb`, `html`, `images`, `layout`, `markdown`, `nlp`, `polars`.
